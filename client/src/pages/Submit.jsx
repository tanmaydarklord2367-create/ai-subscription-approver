...
export default function Submit() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) setSubmitted(data.request);
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div className="card" style={{ maxWidth: 500, width: '100%', padding: 48, textAlign: 'center', position: 'relative' }}>
            <div style={{
              ...
              fontSize: 32, margin: '0 auto 20px', boxShadow: 'var(--shadow-col)',
            }}>🎉</div>
            <h2 style={{ fontSize: 26, fontWeight: 800, marginBottom: 10, letterSpacing: '-0.02em' }}>Request Submitted!</h2>
            <p style={{ color: 'var(--text-secondary)', marginBottom: 28, lineHeight: 1.65, fontSize: 15 }}>
              Your request for <strong>{submitted.toolName}</strong> is now pending HR review. You'll hear back soon.
            </p>
            <div style={{ background: 'var(--bg)', borderRadius: 'var(--radius)', padding: '12px 16px', marginBottom: 28, textAlign: 'left', border: '1px solid var(--border)' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: 1.5, marginBottom: 4 }}>REQUEST ID</div>
              <code style={{ fontSize: 12, color: 'var(--text-secondary)', wordBreak: 'break-all' }}>{submitted.id}</code>
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button className="btn btn-outline" onClick={() => { setSubmitted(null); setForm(EMPTY_FORM); }}>Submit Another</button>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '44px 24px 60px', position: 'relative' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 30, fontWeight: 900, letterSpacing: '-0.02em', lineHeight: 1.2 }}>
            Submit a <span className="gradient-text">Tool Request</span>
          </h1>
          <p style={{ color: 'var(--text-secondary)', marginTop: 8, fontSize: 15 }}>
            HR reviews it first, then the director gives final approval.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }} className="grid-2">
              <div className="form-group">
                ...
                <input className="form-input" value={form.employeeName} onChange={set('employeeName')} placeholder="Jane Smith" required />
              </div>
              <div className="form-group">
                <label className="form-label">Work Email <span className="req">*</span></label>
                <input className="form-input" type="email" value={form.employeeEmail} onChange={set('employeeEmail')} placeholder="jane@django.agency" required />
              </div>
            </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  ...
                  <button
                    ...
                    style={{
                      padding: '13px 14px', textAlign: 'left', fontFamily: 'inherit', cursor: 'pointer',
                      ...
                      boxShadow: form.toolType === t.value ? '0 4px 12px rgba(99,102,241,0.15)' : 'none',
                    }}
                  >
                    <div style={{ fontSize: 22, marginBottom: 5 }}>{t.icon}</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: form.toolType === t.value ? 'var(--primary)' : 'var(--text-primary)' }}>{t.label}</div>
            ...
            <div className="form-group">
              ...
              <input className="form-input" value={form.toolName} onChange={set('toolName')} placeholder="e.g. GitHub Copilot, Notion AI, Jasper" required />
            </div>

            <div className="form-group">
              <label className="form-label">Tool Website <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(optional)</span></label>
              <input className="form-input" type="url" value={form.toolWebsite} onChange={set('toolWebsite')} placeholder="https://…" />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }} className="grid-2">
              <div className="form-group">
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input className="form-input" style={{ paddingLeft: 26 }} type="number" min="0" step="0.01" value={form.budgetAmount} onChange={set('budgetAmount')} placeholder="999" required />
                </div>
              </div>
              <div className="form-group">
            ...
            <div className="form-group">
              ...
              <textarea className="form-textarea" value={form.reason} onChange={set('reason')} placeholder="Describe your use case, expected productivity gains, and why this tool is needed…" required rows={5} maxLength={1000} />
